...
class DayOverviewViewModel(
    ...
    private val getDaySummaryCached = StateFlowCache<LocalDate, Cached<FullDaySyncable>?>{
        date -> repos.daySummaryRepo.getDaySummary(date).subscribeToColdFlow(viewModelScope, null)
    }
    ...
    fun getStepsFor(it: LocalDate) = repos.stepRepo.getStepsCached(it)
