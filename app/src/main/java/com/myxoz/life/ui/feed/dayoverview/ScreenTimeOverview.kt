...
@Composable
fun ScreenTimeOverview(screenTimeOverviewModel: ScreenTimeOverviewModel) {
    val date = screenTimeOverviewModel.date
    val isToday = remember { LocalDate.now() == date }  // TODO Adjust to today flow
    val daySummary by screenTimeOverviewModel.daySummary.collectAsState()
    var showsMore by remember { mutableStateOf(isToday) }
    val displayedEntries by if(showsMore) {
        screenTimeOverviewModel.liveScreenTime
    } else {
    ...
    val accumulatedTime by screenTimeOverviewModel.screentimeFlow.collectAsStateWithLifecycle(0L)
